...
private:
	...
	void createDescriptorSetLayout() {
		...
		if (vkCreateDescriptorSetLayout(device, &descriptorInfo, nullptr, &descriptorLayout) != VK_SUCCESS) {
	...
	void createVertexBuffer() {
		...
		createBuffer(bufferSize, VK_BUFFER_USAGE_TRANSFER_SRC_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, stageBuffer, stagingBufferMemory);
	...
	void recordCommandBuffer(VkCommandBuffer commandBuffer, uint32_t imageIndex) {
		...
		if (vkEndCommandBuffer(commandBuffer) != VK_SUCCESS) {
			throw std::runtime_error("failed to create record command buffer");
		}
	}

	void createSyncObects() {
	...
	void updateUniformBuffers(uint32_t currentFrame) {
		...
		ubo.proj = glm::perspective(glm::radians(45.0f), (float)swapChainExtent.width / (float)swapChainExtent.height, 0.01f, 100.0f);
		ubo.proj[1][1] *= -1;
